...
$TMP/job-list | grep -q '^DIR.*TASK.*STATE.*REQUEST.*UPDATED (JST)$'
if "$TMP/job-status" >/dev/null 2>&1; then echo "job-status accepted a missing job ID" >&2; exit 1; fi

now=$(date +%s)
write_status() {
  id=$1 state=$2 project=$3 task=$4 updated=$5 heartbeat=$6
  {
    printf 'state=%s\nid=%s\nproject=%s\ntask=%s\nupdated=%s\n' "$state" "$id" "$project" "$task" "$updated"
    [ -z "$heartbeat" ] || printf 'heartbeat_epoch=%s\n' "$heartbeat"
  } > "$WORK/.remote/status/$id.status"
}
write_status newest-running running AAA alpha 2026-08-15T10:31:00Z "$now"
write_status old-running running BBB beta 2026-08-15T09:45:00Z 1
write_status done-job finished CCC/DDD gamma 2026-08-15T09:15:00Z 1
write_status error-job error EEE delta 2026-08-15T08:42:00Z ''
write_status cancel-job cancelled FFF epsilon 2026-08-15T08:00:00Z 1
write_status queued-job queued GGG zeta 2026-08-15T07:30:00Z ''
write_status cancelling-job cancelling HHH eta 2026-08-15T07:00:00Z "$now"
queued_id=20260815T071500Z-queued-without-status
printf 'schema_version=2\nproject=QUEUED\ntask=queued-task\n' > "$WORK/.remote/requests/$queued_id.request"

list=$($TMP/job-list)
printf '%s\n' "$list" | sed -n '2p' | grep -q 'AAA.*alpha.*running.*newest-running'
printf '%s\n' "$list" | grep -q 'BBB.*beta.*stale.*old-running'
...
printf '%s\n' "$list" | grep -q "QUEUED.*queued-task.*queued.*$queued_id.*2026-08-15 16:15 JST"
...
$TMP/job-status "$queued_id" | grep -q '^task=queued-task$'
