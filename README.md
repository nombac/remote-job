...
`remote-job` は、Dropbox、OneDrive、Google Drive、社内ファイルサーバーなど、既に利用できる同期フォルダをジョブキューとして使うツールです。対象プロジェクトのディレクトリでタスク名を付けて `job-submit` を実行すると、ワーカー側のMacが同じプロジェクトの `./run <task>` を1回実行します。依頼後は、実行状況とログの確認や処理のキャンセルができます。

## 必要条件と構成

- macOS 13以降
- 両方のMacにローカルマウントされた、同期対象の `WORK_ROOT`
- 実行可能な各プロジェクトが `WORK_ROOT` 以下のディレクトリであり、タスク名を1引数として受け取る実行可能な `./run` を含んでいること
...
ジョブを投入するには、対象プロジェクトのディレクトリへ移動して、タスク名を1つ指定して `job-submit` を実行します：

```sh
cd "$HOME/Dropbox/Work/simulations/case-01"
job_id=$(job-submit simulation-full)
job-status "$job_id"
job-list                         # 待機中を含むジョブ一覧（新しい順）
```

`job-submit` はタスク名を正確に1つ要求し、対象プロジェクトのディレクトリ内で実行する必要があります。タスク名は英数字で始まり、英数字、`.`、`_`、`-` だけで構成します。`job-cancel <request-id>` と `job-delete <request-id>` は指定したジョブだけを対象とし、`job-status <request-id>` と `job-list` と同様に任意のディレクトリから実行できます。キャンセル対象がすでに終了状態にある場合は、安全に何も行いません。

`job-submit` は投入ごとに `WORK_ROOT/.remote/requests/<request-id>.request` を次の形式でアトミックに作成します：

```text
schema_version=2
project=simulations/case-01
task=simulation-full
```
...
```text
DIR                      TASK                           STATE       REQUEST                                               UPDATED (JST)
AAA                      simulation-full                running     20260815T193100JST-...                                 2026-08-15 19:31 JST
BBB                      build-release                  finished    20260815T181500JST-...                                 2026-08-15 18:15 JST
CCC/DDD                  analysis                       error       20260815T174200JST-...                                 2026-08-15 17:42 JST
```
...
```text
Mac B：対象プロジェクトのディレクトリでjob-submit <task>
  ↓
ワーカー側のMacを起動したまま帰宅
  ↓
Mac A：job-list
...
Mac A：job-cancel <request-id>（必要な場合）
  ↓
完了後：対象プロジェクトのディレクトリで次のjob-submit <task>
```
...
```text
Mac A：対象プロジェクトのディレクトリでjob-submit <task>
  ↓
Mac B：ワーカーが./run <task>を実行
  ↓
...
id=$(job-submit simulation-full)
job-status "$id"       # queued → running → finished/error
# job-cancel "$id"     # 必要な場合
```

同梱の [`example-project`](example-project) は、必須となるエントリーポイントの例です。実行可能な `run` は受け取ったタスク名を `make <task>` へ渡します。

### ハートビートと `stale`

`./run <task>` の実行中、ワーカーはステータス内の `heartbeat_epoch` を20秒ごとにアトミックに更新します。`job-list` と `job-status` は、`running` または `cancelling` のハートビートが180秒以上前のものである場合、ローカルで `stale` と判定します。同期されたステータスファイル自体は書き換えません。`job-status` では元の値も `reported_state` として表示します。
...
`stale` では、計算プロセスが監視なしで動いている、すでに終了または停止している、ワーカーMacが停止している、同期が遅延している、ステータスが不正である、といった複数の可能性を区別できません。そのため `job-delete` は `stale` を拒否します。

`stale` に `job-cancel <request-id>` を実行すると、ワーカーは記録されたプロセスグループをワーカーMac上で確認します。プロセスグループが存在しなければ、安全に停止済みと判断して状態を `cancelled` に確定します。プロセスグループが残っている場合は、監視外のプロセスを誤って停止しないよう `stale` のまま維持します。LaunchAgentの再起動は不要であり、再起動してもstaleジョブへの監視は復旧しません。状態が `cancelled` になった後は `job-delete` で削除できます。

### キャンセルの動作

各 `./run <task>` は、それぞれ独立したPOSIXプロセスグループ内で直接開始されます。キャンセル時、ワーカーは状態を `running` から `cancelling` にアトミックに変更し、**プロセスグループ全体**に `SIGTERM` を送信して最大5秒待機します。その後もプロセスが残っていれば、グループに `SIGKILL` を送信します。これにより、親プロセスだけでなく、一般的な `./run <task> → make <task> → Python/Wolfram/その他の計算処理` というプロセスツリー全体が停止します。終了状態は `error` とは区別される `cancelled` です。
...
- ワーカーはrootでの実行を拒否し、プロジェクトに固定されたエントリーポイント `./run <task>` だけを実行します。リクエストに含まれるのは任意のコマンドではなく、プロジェクトパスと検証済みタスク名です。タスク名は独立した1引数として渡し、`sh -c` は使用しません。
...
- `./run <task>` はワーカーユーザーの権限で実行され、ワーカーの環境を継承します。これは任意の信頼済みコードです。`.remote/requests` と `WORK_ROOT` 以下のプロジェクトファイルの両方に書き込める人は、そのコードを実行させることができます。信頼できない共有フォルダでは使用しないでください。
